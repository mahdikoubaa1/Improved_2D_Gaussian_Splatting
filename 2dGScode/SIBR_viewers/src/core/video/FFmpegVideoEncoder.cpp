...
#pragma warning(suppress : 4996)
	...
	void FFVideoEncoder::close()
	{
		...
		if (video_st) {
    	    // 1. Use the pCodecCtx variable directly instead of video_st->codec
    	    if (pCodecCtx) {
    	        avcodec_free_context(&pCodecCtx);
    	    }
		
    	    // 2. Use the modern frame free function
    	    if (frameYUV) {
    ...
    {
        // 1. Send the frame to the encoder
        // Note: Using 'frameYUV' (your internal scratch frame) as the input
        ...
        // 2. Poll for encoded packets
        // Encoders may output 0, 1, or multiple packets for a single input frame
        while (ret >= 0) {
            ret = avcodec_receive_packet(pCodecCtx, pkt);

            // EAGAIN means "need more input frames to produce a packet"
            // EOF means the encoder is flushed/closing
            ...
            // 3. Successfully got an encoded packet
            pkt->stream_index = video_st->index;
            
            // Rescale timing from the codec timebase to the file (container) timebase
            av_packet_rescale_ts(pkt, pCodecCtx->time_base, video_st->time_base);

            // Write the packet to the output file
            ret = av_interleaved_write_frame(pFormatCtx, pkt);
            
            // Wipe the packet for reuse
            av_packet_unref(pkt);

            if (ret < 0) {
                SIBR_WRG << "[FFMPEG] Failed to write frame to file." << std::endl;
                return false;
            }
        }
